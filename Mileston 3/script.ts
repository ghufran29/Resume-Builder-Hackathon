...
function generateResume(): void {
  ...
  const requiredFields = form.querySelectorAll<HTMLInputElement | HTMLTextAreaElement>("input[required], textarea[required]");
  ...
  const educationEntries = document.querySelectorAll<HTMLElement>("#educationSection .educationEntry");
  const educationHTML = Array.from(educationEntries)
    .map((entry) => {
      ...
      const dates = (entry.querySelector('input[name="educationDates"]') as HTMLInputElement).value;
  ...
  const experienceEntries = document.querySelectorAll<HTMLElement>("#experienceSection .experienceEntry");
  const experienceHTML = Array.from(experienceEntries)
    .map((entry) => {
      ...
      const expDates = (entry.querySelector('input[name="experienceDates"]') as HTMLInputElement).value;
      const expDetails = (entry.querySelector('textarea[name="experienceDetails"]') as HTMLTextAreaElement).value;
...
function printResume(): void {
  ...
  const resumeContent = (document.getElementById("resume") as HTMLElement).outerHTML;

  document.body.innerHTML = resumeContent;
  window.print();
  document.body.innerHTML = originalContent;

  (document.getElementById("formContainer") as HTMLElement).style.display = "none";
  ...
  (document.getElementById("printBottom") as HTMLElement).style.display = "block";
}
